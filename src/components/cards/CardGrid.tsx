  ...
  return (
    ...
    >
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="rounded-lg border border-line p-2 animate-pulse">
          <div className="aspect-[63/88] w-full rounded-md bg-surface-muted-2" />
          <div className="mt-2 h-3 w-3/4 rounded bg-surface-muted-2" />
          <div className="mt-1 h-3 w-1/2 rounded bg-surface-muted-2" />
        </div>
      ))}
    </div>
  );
